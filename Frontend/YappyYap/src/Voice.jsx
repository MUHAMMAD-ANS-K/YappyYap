...
export default function Voice() {
    ...
    const moveBarAnimation = useRef()
    const moveBarMover = useRef()
    ...
    useGSAP(() => {
        ...
        barMover.current = (cls, d) => {
            moveBarAnimation.current = gsap.to(`.${cls}`, {
                width: "100%",
                stagger: d,
                ease: "linear",
                paused: true
        ...
        dotMover.current = (cls, d) => {
            moveBarMover.current = gsap.to(`.${cls}`, {
    ...
    useEffect(() => {
        ...
        websocket.current = new WebSocket("ws://localhost:8000/voice")
        ...
        websocket.current.onmessage = (e) => {
            ...
            document.querySelector(".audio-play").src = window.URL.createObjectURL(e.data)
            console.log(e.data)
    ...
    return (
        <div className="audio"><audio className="audio-play" src={src} />
            ...
            <div className="msg-bars-mover"></div>
            <div className="msg-bars">
                {arr.map((element, index) => {
                        <div className="bar" key={index + 3} style={{ height: element }}>
                            <div className="msg-bar-underlay" />
                        </div>
                })}
                <div className="bar" height="100px"></div>
            </div>
        </div>
    )
}
async function play_pause_audio(e) {
    ...
    const movingDot = parent[2].style.transform
    if (movingDot == "translate(0px)" || movingDot == "") {
        let currentDuration = element.duration;
        dotMover.current("msg-bars-mover", currentDuration)
        barMover.current("msg-bar-underlay", currentDuration / 30)
    }
    if (element.paused) {
        parent[1].innerHTML = `
        ...
        moveBarAnimation.current.play();
        moveBarMover.current.play();
        element.play()
    }
    else {
        parent[1].innerHTML = `
        ...
        moveBarMover.current.pause()
        element.pause()
        moveBarAnimation.current.pause()
    }
    element.onended = () => {
        parent[1].innerHTML = `
                    <svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 96 96" xmlSpace="preserve">
        ...
        gsap.set(".msg-bars-mover", { x: 0 })
        document.querySelectorAll(".msg-bar-underlay").forEach(bar => {
            bar.style.width = "0"
        })
    }
}
        ...
        <div className="voice">
            <ul className="msgs">
                <li className="chat-message-block">
                    <img src={default_img} alt="user" className="chat-message-img" />
                    <span>
                        <span className="chat-message-header"><h3 className="username">User</h3><span className="timestamp">12:00am</span></span>
                        {/* <div className="audio" onLoad={helperFunction(audio2)}>
                                                </div>  */}
                        <HelperFunction src={audio2} />
                    </span>
                </li>
                {/* <li className="chat-message-block">
                                    <img src={default_img} alt="user" className="chat-message-img"/>
                                    <span>
                                        <span className="chat-message-header"><h3 className="username">User</h3><span className="timestamp">12:00am</span></span>
                                        <div className="audio">
                                        <audio className="audio-play" src={audio2}/> 
                                            <button className="audio-play-button" onClick={play_pause_audio}>
                                                <Play/>
                                            </button>
                                            <div className="msg-bars-mover"></div>
                                                <div className="msg-bars">
                                                    {helperFunction().map((element, index)=>
                                                            <>
                                                            <div className="bar" key={index + 3} style={{height : element}}>
                                                                <div className="msg-bar-underlay"/>
                                                            </div>
